...
- [x] **P4: ログ単位マージへ昇格** ✅ 実装済み
  ...
  従来方式を置き換え、以下を行う:
  - **UNION（データ損失なし）**: `workoutLogs`/`mealLogs`(idキー)、
    `sleepLogs`/`claimedQuestsByDate`(dateキー)、`claimedAchievements`/
    `favorites`(集合)は両側を合わせて保持する。id/dateが衝突する場合は
    winner側の内容を優先。
  - **派生状態の再計算**: EXP合計/レベル/ステータス/部位ボリュームは、
    各ログに**保存済みの値**（`earnedExp`/`statGains`/`undo.partGain`/
    `undo.bossExp`）を合算するだけで再計算する。ボスの撃破判定・
    ストリークは、保存済み`earnedExp`と日付を時系列順に辿って
    シミュレートする。**生の重量・レップ数からEXP公式を再実行することは
    しない**（種目係数やボスHPテーブルは将来調整されうるため、式を
    再実行すると係数変更時に過去ログの獲得量が静かに変わってしまう。
    保存済みの値だけを合算すれば、その心配がない）。
  - どちらの端末単体でも達成できない結果（合算して初めてボスを倒す/
    ストリークが繋がる）が正しく出ることをテストで検証済み。
  - マージ後の全ログから取り消し(undo)スナップショットを除去する
    （マージ前提のスナップショットはマージ後の状態に対して無効なため。
    次に記録する1件からまた取り消せるようになる）。
  - 完全には再現できない項目（ショップ購入で増減する`gold`/
    `expBoostCharges`/`streakShields`/`playerHp`。購入・ペナルティの
    履歴を保持していないため）は winner の値をそのまま引き継ぐ
    （以前のLWW方式と同じ、今回の既知の割り切り）。
  - `store/cloudSync.ts` の `resolveMerge` を更新し、マージ結果を
    ローカルに適用してからremoteへも書き戻すように変更。
  - テスト14件追加（`domain/mergeState.test.ts`）。

## 5. 既知のトレードオフ（今回の割り切り）

- gold/expBoostCharges/streakShields/playerHp は、マージ時も引き続き
  winner（進捗ガードで決めた側）の値をそのまま引き継ぐ。ショップ購入や
  日次ペナルティの履歴を保持していないため、ログからの完全な再構成が
  できない。実害は小さい（消耗品の残数・ライブなHPという性質上、
  記録データ本体ほど重要ではない）。
